...
{
    public class when_only_a_first_name_is_provided
    {
        public AddPersonHandler Subject;
        public Mock<IStorage> Storage;

        public when_only_a_first_name_is_provided()
        {
            Storage = new Mock<IStorage>();

            Subject = new AddPersonHandler(Storage.Object);

            Subject.Handle(new AddPerson { FirstName = "Bob" });
        }

        [Fact]
        public void person_is_created()
        {
            Storage.Verify(s => s.Add(It.Is<Person>(p => p.FirstName == "Bob")));
        }
    }
}